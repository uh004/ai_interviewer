...
def summarize_interview(state: State) -> State:
    ...
    for s in order:
        ...
        log(f"\n[{s}] 종합 평가 (질문 {n}개 기반)")
        log("-" * 60)
        log(" 항목별 평균(0~1): " + ", ".join([f"{k} {avg[k]:.2f}" for k in crits]))
        log(f" ▶ 종합 점수: {overall}/10")
        if strengths:
            log(f" ▶ 강점: {', '.join(strengths)}")
        if weaknesses:
            log(f" ▶ 개선 필요: {', '.join(weaknesses)}")

        if s == "경험":
            log(" ▸ 조언: 수치·역할·방법을 STAR(상황-과제-행동-결과) 구조로 일관되게 제시하세요.")
        elif s == "동기":
            log(" ▸ 조언: ‘왜 우리/왜 지금/왜 나’를 2~3문장 핵심 연결로 선명하게 설명하세요.")
        elif s == "논리":
            log(" ▸ 조언: 원인→대안→선택 근거→결과의 인과 뼈대를 먼저 말하고 사례를 덧붙이세요.")
    ...
    if total_scores:
        ...
        log("\n [인터뷰 전체 종합 평가]")
        log("=" * 60)
        log(f" 평균 점수: {avg_score}/10\n")

        final_feedback = final_eval_chain.invoke({
            "avg_score": avg_score,
            "summary_for_llm": "\n".join(summary_for_llm_lines)
        })
        log(f" {final_feedback}")
    else:
        log("\n 종합 점수를 계산할 수 있는 평가 데이터가 부족합니다.")

    log("=" * 60)
    log(" 인터뷰가 종료되었습니다. 수고하셨습니다!\n")

    state["next_step"] = "end"
    state["final_report"] = report  # ✅ 이게 핵심
    return state